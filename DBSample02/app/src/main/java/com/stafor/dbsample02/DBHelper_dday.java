...
public class DBHelper_dday extends SQLiteOpenHelper {
    ...
    // Database name
    private static final String DATABASE_NAME = "Do_dday.db";
    // Database version
    private static final int DATABASE_VERSION = 1;
    // Table name
    private static final String TABLE_NAME = "ddayTable";

    //DBHelper 생성자(Context, DBname, cursor, DBversion)
    public DBHelper_dday(Context context) {
    ...
    *  - 새로운 데이터 베이스를 생성할때 사용*/
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + TABLE_NAME + "( _id INTEGER PRIMARY KEY AUTOINCREMENT, year INTEGER, month INTEGER, day INTEGER, content TEXT);");
    ...
    *  - DB를 변경하고, 버전읇 변경할때 여러가지 업그레이드 작업 수행 가능*/
    ...
    // 테이블의 레코드 insert
    public void insert(int year, int month, int day, String content) {
        db = getWritableDatabase();

        db.execSQL("INSERT INTO " + TABLE_NAME + " VALUES(null, " + year + ", " + month + ", " + day + ", '" + content + "');");
        db.close();
    }
