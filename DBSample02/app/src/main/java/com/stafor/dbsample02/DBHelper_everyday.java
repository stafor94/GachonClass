...
//매일할일 DBHelper 클래스
public class DBHelper_everyday extends SQLiteOpenHelper {
    ...
    // Database name
    private static final String DATABASE_NAME = "Do_everyday.db";
    // Database version
    private static final int DATABASE_VERSION = 1;
    // Table name
    private static final String TABLE_NAME = "everydayTable";

    //DBHelper 생성자(Context, DBname, cursor, DBversion)
    public DBHelper_everyday(Context context) {
    ...
    *  - 새로운 데이터 베이스를 생성할때 사용*/
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + TABLE_NAME + "( _id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT);");
    ...
    *  - DB를 변경하고, 버전읇 변경할때 여러가지 업그레이드 작업 수행 가능*/
    ...
    public void insert(String content) {
        ...
        db.execSQL("INSERT INTO " + TABLE_NAME + " VALUES(null, '" + content + "');");
        db.close();
    }
    ...
    public String printData5() {
        db = getReadableDatabase();
        String str=" ";

        cursor = db.rawQuery("SELECT content FROM " + TABLE_NAME + ";", null);
        if ( cursor.moveToPosition(4) == false ) {
            str = " ";
        } else {
            str += cursor.getString(0);
        }
    ...
    public String printData6() {
        db = getReadableDatabase();
        String str=" ";

        cursor = db.rawQuery("SELECT content FROM " + TABLE_NAME + ";", null);
        if ( cursor.moveToPosition(5) == false ) {
            str = " ";
        } else {
            str += cursor.getString(0);
        }
    ...
    public String printData_last() {
        db = getReadableDatabase();
        String str="";

        cursor = db.rawQuery("SELECT _id FROM " + TABLE_NAME + ";", null);
        cursor.moveToLast();
        str += cursor.getInt(0);
        cursor.close();
        return str;
